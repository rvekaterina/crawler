...
func (c *crawlerImpl[T, R]) Collect(
	...
	go func() {
		defer wg.Done()
		wp2.List(ctx, conf.SearchWorkers, file{root, true}, searcher(fileSystem, files, errCtx, &once))
		close(files)
	}()

	transformedFiles := wp2.Transform(ctx, conf.FileWorkers, files, transform[T](fileSystem, errCtx, &once))
